 ...
 * Déssine un triangle
 */
export class Mercure extends AbstractForm {
  ...
  Mercure(ctx, dx, dy) {
    ...
    radial.addColorStop(1, "#DEBD7B"); //Rouge

    ctx.beginPath();
    ctx.arc(ox + 50, oy + 325 - 150, 20, 0, Math.PI * 2, true); // Mercure
    ...
    for (let i = 0; i <= ~~(Math.random() * 59); i++)
      ctx.arc(
        ox + ~~(Math.random() * 20) + 40,
        oy + ~~(Math.random() * 15) + 325 - 150,
        ~~(Math.random() * 9),
        0,
...
*/
  ...
  draw(ctx) {
    ctx.save();
    this.Mercure(ctx, this.x, this.y);
